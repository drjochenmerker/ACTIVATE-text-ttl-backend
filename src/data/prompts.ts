...
const rawBasePrompt = `
    ...
    "Subject" (individual or group of individuals of focus),
    "Object" (subject’s task or goal), "Instrument" (tools - tangible object and technology used), "Rule" (explicit or implicit rules),
    "Community" (social environments) or "DivisionOfLabour" (task division within the social environment).

    You should also extract the most important conflicts and feedback between the entities. 
    The extracted conflicts and feedback must be assigned a unique identifier, the class "Conflict", a title ("ConflictTitle"),
    a description which must not be a direct quote ("ConflictDescription"), the state ("ConflictState") "open" and an author ("WrittenBy") who must be one of the
    given entities that initiated the conflict/feedback. If entities are involved in a conflict or feedback, they must be linked
    with the relation "HasParticipant" with the conflict/feedback as the source entity. A conflict/feedback must not have paricipants from
    ...
    combinations: (Subject,Instrument, Object), (Subject, Rule, Community), (Object, Community, DivisionOfLabour) or (Subject, Object, Community).
    If the participants belong to just one  or two classes, the classes may be freely chosen.   
    
    If other entities respond to a conflict/feedback, they must be linked by creating a new entity of the "Comment" class and linked
    to the conflict/feedback with the relation "HasComment". The comment must have a unique identifier a description which must not be a direct quote
    ("CommentDescription") and an author ("WrittenBy") who must be one of the entities that responded to the conflict/feedback.
...
const personaBasePrompt = `
    ...
    You should also extract the most important conflicts and feedback between the entities. 
    The extracted conflicts and feedback must be assigned a unique identifier, the class "Conflict", a title ("ConflictTitle"),
    a description which must not be a direct quote ("ConflictDescription"), the state ("ConflictState") "open" and an author ("WrittenBy")
    who must be one of the given entities that initiated the conflict/feedback. If entities are involved in a conflict or feedback, they must be linked
    with the relation "HasParticipant" with the conflict/feedback as the source entity. A conflict/feedback must not have paricipants from
    more than three classes. If the participants belong to exactly three classes, the classes may only be from one of the following 
    combinations: (Subject,Instrument, Object), (Subject, Rule, Community), (Object, Community, DivisionOfLabour) or (Subject, Object, Community).
    If the participants belong to just one  or two classes, the classes may be freely chosen. 
 ...
 * They guide the LLM through the process of extracting entities, setting information, relations, and conflicts/feedback from the input text.
...
export const iterativeSystemPrompts = {
    zeroPlusVocab: [
        ...
        according to their context in the given situation: "Subject" (individual or group of individuals of focus),
        "Object" (subject’s task or goal), "Instrument" (tools - tangible object and technology used), "Rule" (explicit or implicit rules),
        "Community" (social environments) or "DivisionOfLabour" (task division within the social environment).
        ...
        `,
        `
        Given any text, a list of entities and a list of relations, you will extract the most important conflicts and feedback between the entities.
        ...
        The input will be structured as follows:
        Text: "Text to extract conflicts/feedback from"
        Data: '''turtle
        @prefix : <http://activate.htwk-leipzig.de/model#> .
        @prefix owl: <http://www.w3.org/2002/07/owl#> .
        @prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> .
        ...
        Output the conflicts/feedback in the following format, generating Titles and Descriptions in German, English and Swedish for each conflict/feedback:
        '''turtle
        @prefix : <http://activate.htwk-leipzig.de/model#> .
        @prefix owl: <http://www.w3.org/2002/07/owl#> .
        @prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> .

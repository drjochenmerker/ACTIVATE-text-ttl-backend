 ...
 */
export async function requestRoleMapping(
    llm: LLM,
    systemPrompt: string,
...
async function queryCortecs(
    ...
    try {
        ...
        writeToLog(
            logFilename,
            "Cortecs Request with model " + model.modelName,
            completion
        );
        return completion.choices[0].message.content || "error";
    } catch (error) {
        console.error(
            "Error querying Cortecs with model " + model.modelName + ":",
            error
        );
        return "error";
    }
